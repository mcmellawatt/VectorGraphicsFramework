...
TEST(BackgroundColor, BasicCanvas)
{
    ...
    Color actual = canvas.getPixelColor(VG::Point(10, 10));
...
TEST(setPixelOutsideDimensions, BasicCanvas)
{
    ...
    bool pixelLocationOutOfRangeExceptionCaught = false;
    try
    {
        canvas.setPixelColor(VG::Point(105, 205), Color(1, 2, 3));
    }
    catch (const PixelLocationOutOfRange&)
    {
        pixelLocationOutOfRangeExceptionCaught = true;
    }

    CHECK(pixelLocationOutOfRangeExceptionCaught);
}
