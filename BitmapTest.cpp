...
TEST(BitmapReadWrite, Bitmap)
{
    ...
    bitmapHeader.writeFileHeader(outputStream);
    bitmapHeader.writeInfoHeader(outputStream);
    ...
    WindowsBitmapHeader bitmapHeader2;
    bitmapHeader2.readFileHeader(bitmapStream2);
    bitmapHeader2.readInfoHeader(bitmapStream2);
    
    Bitmap bitmap2(bitmapHeader2.getBitmapWidth(), bitmapHeader2.getBitmapHeight(), bitmapStream2);

    int numberOfScanLines = 0;
    for (Bitmap::ScanLineIterator scanLine = bitmap2.begin(); scanLine != bitmap2.end(); ++scanLine)
    {
        CHECK_EQUAL(100, scanLine->size());
        ++numberOfScanLines;
    }
...
TEST(BinaryOstreamIterator_101, Bitmap)
{
    ...
    for (Bitmap::ScanLineIterator scanLine = bitmap.begin(); scanLine != bitmap.end(); ++scanLine)
    {
        ...
        // Write pad bytes
        for (unsigned int pad = 0; pad < bitmap.getNumberOfPadBytes(); ++pad)
        {
